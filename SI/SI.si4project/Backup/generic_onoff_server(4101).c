...
static void handle_set(mesh_model_msg_ind_t const *p_rx_msg)
{
    ...
    if (set_params_validate(p_rx_msg, p_msg_params_packed))
    {
        ...
        if(model_tid_validate(&tid_tracker, p_rx_msg, GENERIC_ONOFF_OPCODE_SET, in_data.tid))
        {
			...
			/*应用层处理*/
			pmu_set_led2_value(in_data.on_off);
            mesh_led_state.on_off_state = in_data.on_off;
...
//                                                            (p_rx_msg->opcode.opcode == GENERIC_ONOFF_OPCODE_SET) ? &out_data : NULL);
            ...
            {
				out_data.present_on_off    = mesh_led_state.on_off_state;
		        out_data.target_on_off     = mesh_led_state.on_off_state;
