...
static S16 Temp_to_Level(U16 Temp)
{
    float tmp;

    /* Mesh Model Specification 6.1.3.1.1 2nd formula start */

    tmp = (Temp - LightStatus.temp.range_min) * UINT16_MAX;

    tmp = tmp / (LightStatus.temp.range_max - LightStatus.temp.range_min);

    return (S16) (tmp + INT16_MIN);
...
/*****************************************************************************
Function name :  Light_Status_Constrain_Value
...
void Light_Status_Default_Init(void)
{
    ...
    LightStatus.LightMode = MEM_DB_MESH.LightMode;
	...
	LightStatus.lightness.last = LightStatus.lightness.last > 0 ? 
	                             Light_Status_Constrain_Value(LightStatus.lightness.last, LightStatus.lightness.range_min, LightStatus.lightness.range_max) : 0;

	LightStatus.lightness.def = LightStatus.lightness.def > 0 ? 
	                             Light_Status_Constrain_Value(LightStatus.lightness.def, LightStatus.lightness.range_min, LightStatus.lightness.range_max) : 0;
								 
	LightStatus.lightness.target = LightStatus.lightness.target > 0 ? 
	                             Light_Status_Constrain_Value(LightStatus.lightness.target, LightStatus.lightness.range_min, LightStatus.lightness.range_max) : 0;
    
	LightStatus.temp.def = Light_Status_Constrain_Value(LightStatus.temp.def, LightStatus.temp.range_min, LightStatus.temp.range_max);							 
	LightStatus.temp.target = Light_Status_Constrain_Value(LightStatus.temp.target, LightStatus.temp.range_min, LightStatus.temp.range_max);		
	...
	MESH_MODEL_LOG("LightStatus.lightness.last = %d\r\n", LightStatus.lightness.last);
	MESH_MODEL_LOG("LightStatus.onpowerup = %d\r\n", LightStatus.onpowerup);
	MESH_MODEL_LOG("LightStatus.lightness.range_min = %d\r\n", LightStatus.lightness.range_min);
}
...
void Light_Status_Set_Target(LIGHT_STATUS_TYPE Type, void *pTarget)
{
    ...
    switch(Type)
    {
        ...
        case LIGHT_STATUS_TYPE_LIGHT_LEVEL:
        {
            LightStatus.lightness.target = *((int16_t *) pTarget) - INT16_MIN;
            LightStatus.lightness.target = LightStatus.lightness.target > 0 ? 
	                             Light_Status_Constrain_Value(LightStatus.lightness.target, LightStatus.lightness.range_min, LightStatus.lightness.range_max) : 0;

            LightStatus.lightness.current = LightStatus.lightness.target;
			
			MESH_MODEL_LOG("lightness.target = %d  lightness.current = %d\r\n", LightStatus.lightness.target, LightStatus.lightness.current);
            
        }break; 
        
        case LIGHT_STATUS_TYPE_ACTUAL:
        case LIGHT_STATUS_TYPE_CTL_LIGHT:
        case LIGHT_STATUS_TYPE_HSL_LIGHT:
            
        {
            LightStatus.lightness.target = *((uint16_t *) pTarget);
            LightStatus.lightness.target = LightStatus.lightness.target > 0 ? 
	                             Light_Status_Constrain_Value(LightStatus.lightness.target, LightStatus.lightness.range_min, LightStatus.lightness.range_max) : 0;
        ...
        case LIGHT_STATUS_TYPE_CTL_TEMP:
        {
             LightStatus.temp.target = *((uint16_t *)pTarget);
             LightStatus.temp.target = Light_Status_Constrain_Value(LightStatus.temp.target, LightStatus.temp.range_min, LightStatus.temp.range_max); 

             LightStatus.temp.current = LightStatus.temp.target;
             
             MESH_MODEL_LOG("temp.target = %d  temp.current = %d\r\n", LightStatus.temp.target, LightStatus.temp.current);

        }break;  

        case LIGHT_STATUS_TYPE_CTL_DELTA_UV:
        {
             LightStatus.temp.delta_target = *((uint16_t *)pTarget);
             LightStatus.temp.delta_current = LightStatus.temp.delta_target;
  
            MESH_MODEL_LOG("temp.delta_target = %d  temp.delta_current = %d\r\n", LightStatus.temp.delta_target, LightStatus.temp.delta_current);

        }break;  

        case LIGHT_STATUS_TYPE_HSL_HUE:
        {
            LightStatus.hue.target = *((uint16_t *)pTarget);
            LightStatus.hue.current = LightStatus.hue.target;
            
            MESH_MODEL_LOG("hue.target = %d  hue.current = %d\r\n", LightStatus.hue.target, LightStatus.hue.current);
 
        }break;  

        case LIGHT_STATUS_TYPE_HSL_SATURATION:
        {
            LightStatus.saturation.target = *((uint16_t *)pTarget);
            LightStatus.saturation.current = LightStatus.saturation.target;
            
            MESH_MODEL_LOG("saturation.target = %d  saturation.current = %d\r\n", LightStatus.saturation.target, LightStatus.saturation.current);
 
        }break;  

        case LIGHT_STATUS_TYPE_HUE_LEVEL:
        {
            LightStatus.hue.target = *((int16_t *)pTarget) - INT16_MIN;
            LightStatus.hue.current = LightStatus.hue.target;
        ...
        case LIGHT_STATUS_TYPE_SATURATION_LEVEL:
        {
            LightStatus.saturation.target = *((int16_t *)pTarget) - INT16_MIN;
            LightStatus.saturation.current = LightStatus.saturation.target;
...
S32 Light_Status_Get_Current(LIGHT_STATUS_TYPE Type)
{
    ...
    switch(Type)
    {
        ...
        case LIGHT_STATUS_TYPE_LIGHT_LEVEL:
        {
             Current = (LightStatus.lightness.current + INT16_MIN);

        }break;           

        case LIGHT_STATUS_TYPE_ACTUAL:
        {
             Current = LightStatus.lightness.current;

        }break; 

        case LIGHT_STATUS_TYPE_TEMP_LEVEL:
        {
             Current = Temp_to_Level(LightStatus.temp.current);

        }break; 
        
        case LIGHT_STATUS_TYPE_CTL_TEMP:
        {
             Current = LightStatus.temp.current;

        }break;  

        case LIGHT_STATUS_TYPE_CTL_DELTA_UV:
        {
             Current = LightStatus.temp.delta_current;
        ...
        case LIGHT_STATUS_TYPE_HSL_HUE:
        {
             Current = LightStatus.hue.current;

        }break;
        
        case LIGHT_STATUS_TYPE_HSL_SATURATION:
        {
             Current = LightStatus.saturation.current;

        }break;  

        case LIGHT_STATUS_TYPE_HUE_LEVEL:
        {
            Current = (int16_t)LightStatus.hue.current + INT16_MIN;

        }break;  
        
        case LIGHT_STATUS_TYPE_SATURATION_LEVEL:
        {
            Current = (int16_t)LightStatus.saturation.current + INT16_MIN;
...
S32 Light_Status_Get_Target(LIGHT_STATUS_TYPE Type)
{
    ...
    switch(Type)
    {
        ...
        case LIGHT_STATUS_TYPE_LIGHT_LEVEL:
        {
             Target = (LightStatus.lightness.target + INT16_MIN);

        }break;  
        
        case LIGHT_STATUS_TYPE_ACTUAL:
        {
             Target = LightStatus.lightness.target;

        }break; 

        case LIGHT_STATUS_TYPE_TEMP_LEVEL:
        {
             Target = Temp_to_Level(LightStatus.temp.target);

        }break; 
        
        case LIGHT_STATUS_TYPE_CTL_TEMP:
        {
             Target = LightStatus.temp.target;

        }break;  

        case LIGHT_STATUS_TYPE_CTL_DELTA_UV:
        {
             Target = LightStatus.temp.delta_target;
        ...
        case LIGHT_STATUS_TYPE_HSL_HUE:
        {
             Target = LightStatus.hue.target;

        }break;  
        
        case LIGHT_STATUS_TYPE_HSL_SATURATION:
        {
             Target = LightStatus.saturation.target;
