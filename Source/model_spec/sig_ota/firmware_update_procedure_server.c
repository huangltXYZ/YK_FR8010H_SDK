...
#define APPLY_RESET_TIMEOUT      (TmrSeconds(3))/*延时3S复位*/
os_timer_t Ota_Apply_Reset_Timer;
static void ota_apply_reset_cb(void *arg);


static void ota_apply_reset_cb(void *arg)
{
    firmware_update_server_t *p_server = (firmware_update_server_t *)arg;
    ...
    /*保存当前开关灯状态*/
    gst_AppMemDB.OTAWithOff = APP_Light_GetOnoff() ? 0 : 1;
    APP_MEM_Fast_Save();
    SYSTEM_RESET();
...
{
    bool check_metadata_passed = false;
...
//	MESH_MODEL_LOG("p_in->fw_id.version_id = %04x\n", p_in->fw_id.version_id);
    ...
    {
        check_metadata_passed =  true;/*富芮坤平台协议栈和APP是合在一个BIN文件的，所以没有协议栈兼容性问题*/
    }
...
//
		...
		os_timer_start(&Ota_Apply_Reset_Timer, APPLY_RESET_TIMEOUT, false);	
    }
...
void firmware_update_init(firmware_update_server_t * p_server)
{
    ...
    os_timer_init(&Ota_Apply_Reset_Timer, ota_apply_reset_cb, p_server);	
