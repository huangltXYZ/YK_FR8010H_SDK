...
__attribute__((section("ram_code"))) void patch_init(void)
{
    ...
    for(i=patch_num; i!=0; )
    {
        ...
        {
            //if(i == 4 )  //if you want to use jlink to do debug, not enable patch4
                //FPB_CompUnset(i);
            //else
...
#endif	
	co_printf("offset:%d\r\n",offset);
	co_printf("image_size:%d\r\n",__jump_table.image_size);
